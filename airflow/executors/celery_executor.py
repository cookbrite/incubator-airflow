...
class CeleryExecutor(BaseExecutor):
    ...
    def sync(self):
        ...
        for key, asyncv in list(self.tasks.items()):
            try:
                state = asyncv.state
                if self.last_state[key] != state:
                    if state == celery_states.SUCCESS:
                        ...
                        self.log.info("Unexpected state: %s", asyncv.state)
                    self.last_state[key] = asyncv.state
    ...
    def end(self, synchronous=False):
        if synchronous:
            while any([
                    asyncv.state not in celery_states.READY_STATES
                    for asyncv in self.tasks.values()]):
                time.sleep(5)
        self.sync()
